...
contract Edubukesealer {
    ...
    mapping(address => mapping(address => string[])) private externalAccessRec;

    mapping(string=>accessReciever[]) private shareAccessList;
    
    //mapping(uint256 => mapping(address=>bytes32[])) private studentRecByClgID;
    ...
    function bulkUpload(bulkuploaddata[] memory data, string memory _issuerName)
        external
        onlyInstitute
    {
    ...
    function updateCertificateURI(string memory _hash, string memory _uri)
        external
        eitherInstituteOrOwner
    {
        bytes32 byte_hash = stringToBytes32(_hash);
        require(
            certificates[byte_hash].timestamp != 0,
    ...
    function updateBulkCertificateURI(bulkuploaddata[] memory data)
        external
        eitherInstituteOrOwner
    {
        for (uint256 i = 0; i < data.length; i++) {
    ...
    function Viewcertificatedata(string memory _hash)
        external
        view
        returns (
            string memory,
    ...
    function getInstituteWitnesses(address _witness)
        external
        view
        eitherInstituteOrOwner
    ...
    function viewCertificateURI(string memory _hash)
        external
        view
    ...
    //14 may added
    function chkApprovedInstitute(string memory _hash)
        external
        view
        returns (bool)
    {
    ...
    function getStudentInfo(address _studentAdd)
        external
        view
    ...
    function shareAccess(string memory _name, address _recvAddress, string memory _recvEmail, string memory _uri) external
    {
      require(studentInfo[msg.sender].studentAdd == msg.sender,"you are not reg stud");
      shareAccessList[_uri].push(accessReciever(_name, _recvAddress, _recvEmail, true));
    }

    function removeAccess(string memory _uri, address _address) external
    {
      require(studentInfo[msg.sender].studentAdd == msg.sender,"you are not reg stud");
      uint len = shareAccessList[_uri].length;
      for(uint i=0 ; i<len ; i++) 
      {
          if(shareAccessList[_uri][i].recvAddress == _address) 
          {
            shareAccessList[_uri][i].accessFlag = false;
          }
      }
    }

    function removeReciever(string memory _uri, address _address) external 
    {
      require(studentInfo[msg.sender].studentAdd == msg.sender,"you are not reg stud");
      uint len = shareAccessList[_uri].length;
      for(uint i=0 ; i<shareAccessList[_uri].length ; i++) 
      {
          if(shareAccessList[_uri][i].recvAddress == _address) 
          {
            shareAccessList[_uri][i] = shareAccessList[_uri][len-1];
            shareAccessList[_uri].pop();
            break;
          }
      }
    }

    function getSharedList(string memory _uri) external view returns (accessReciever[] memory)
    {
        require(studentInfo[msg.sender].studentAdd == msg.sender,"you are not reg stud");
        return shareAccessList[_uri];
    }
